		...
		menuItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ae){
            	if(JOptionPane.showConfirmDialog(tree,"Delete "+curNode,"Delete File",JOptionPane.ERROR_MESSAGE)==JOptionPane.YES_OPTION){
            		if(((File)curNode.getUserObject()).isFile())((File)curNode.getUserObject()).delete();
            		else DelDir((File)curNode.getUserObject());
		...
		menuItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ae){
                ...
                DefaultMutableTreeNode newNode=new DefaultMutableTreeNode(new File(
              	...
              	if(lastSelNode.getChildCount()!=0)newNode.add(new DefaultMutableTreeNode("**"));                
            	treeModel.insertNodeInto(newNode,(MutableTreeNode)tree.getLastSelectedPathComponent(),0);
            	if(cut)treeModel.removeNodeFromParent(lastSelNode);
            	tree.repaint();
            }
        });
	}
	public void show(JTree comp, Point p, DefaultMutableTreeNode node){
		if(comp!=null&p!=null&node!=null){
			...
			if(lastSelNode!=null&((File)node.getUserObject()).isDirectory()){
				if(!lastSelNode.isNodeDescendant(curNode))menuItem.setEnabled(true);
			}
